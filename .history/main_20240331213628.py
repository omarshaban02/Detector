...
class Application(QMainWindow, Ui_MainWindow):
    ...
    def setup_hough_sliders(self):
        ...
        for hough_slider, hough_param in zip(self.dict_hough_sliders.keys, self.dict_circle_sliders.keys):
            hough_parameter = self.dict_hough_sliders[hough_slider]
            hough_slider.valueChanged.connect(lambda value: hough_parameter = value)
            
    def update_parameters(self):
        line_params = [self.line_rho, self.line_theta, self.line_thresh]
        circle_params = [self.circle_min_radius, self.circle_max_radius, self.circle_threshold, self.circle_min_dist]
        for slider, line_edit in self.dict_hough_sliders.items():
            slider.valueChanged.connect(lambda value, line_edit=line_edit: line_edit.setText(str(value)))
        
        for hough_slider, circle_slider ,line_param, circle_param in zip(self.dict_hough_sliders.keys, self.dict_circle_sliders.keys, line_params, circle_params):
