...
class Application(QMainWindow, ui):
    ...
    ############################################ Connections ###################################################
        self.btn_start_contour.clicked.connect(self.start_oreder)
        self.sigma_slider.valueChanged.connect(self.hough_line_transform)
        self.low_threshold_slider.valueChanged.connect(self.hough_line_transform)
        self.high_threshold_slider.valueChanged.connect(self.hough_line_transform)
    ...
    def hough_line_transform(self):
        ...
        min_thresh = 0.05
        max_thres =  0.07
        edge_image = self.hough_obj.canny_edge_detection( 1.0,min_thresh ,max_thres)
        if edge_image is not  None:
            num_rho, num_theta, bin_threshold = self.hough_obj.tune_parameters(edge_image)
